...
- linking the global activity log to the associated thing, for example, the provider
...
- linked accreditations - in a list showing the accredited provider number and accreditation dates
...
We remove links to providers if they have been deleted from the register. We do not delete items from the activity log, as it is an record which cannot be altered or changed.
...
We do not know how long we will need to retain the activity logs, but it could be as much as 7 years’ worth of changes.
