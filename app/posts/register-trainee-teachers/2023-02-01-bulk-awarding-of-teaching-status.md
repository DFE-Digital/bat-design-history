...
Although SCITTs only use Register to work with trainee teacher data, HEIs still need to use several different systems. We would like to reduce this to just 2 systems, HESA and Register.

To support this, we intend to turn off the TRA’s ITT provider portal. This means that any user needs it currently meets will need to be met by Register.

An HEI might need to recommend up to 500 trainees at one time for teaching status. We believe it will take too much time for providers to do this one by one, so we need a bulk solution in Register.
...
- only allowing teaching status recommendations, rather than a range of status changes
...
If there are no errors then the data is processed that night. The provider gets an email the next day telling them that their upload was successful.
...
- cannot be recommended because there are errors in the row

Since the spreadsheet we provide to users will not have any errors, it should only possible for the upload to have errors if the user:
